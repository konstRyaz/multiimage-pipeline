...
| --- | --- | --- | ---: | ---: | --- | --- |
| `smoke` | 64 train / 32 val | по 5 пар каждого класса на блок | до 64 | до 32 | запрещён | `prepare → calibrate → validate` |
...
`smoke` — только техническая проверка полного маршрута, модели, порогов,
метрик, манифестов и кэша. Его WIDER FACE и XQLFW выбираются детерминированно,
а результаты явно помечаются `technical_smoke_only=true`; использовать их как
исследовательские метрики нельзя. Профили `dev-a`, `dev-b` и `full` по-прежнему
обрабатывают WIDER FACE и XQLFW полностью.
Версия `technical_smoke_profile_v2` входит в отпечаток эксперимента, поэтому
кэш прежнего ошибочного `smoke` не считается совместимым.
