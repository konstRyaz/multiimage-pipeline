...
Установщик готовит WIDER FACE validation, исходный невыравненный XQLFW и,
...
- WIDER FACE: 3226 изображений, разметка рамок и файлы подмножеств
  Easy/Medium/Hard;
...
Эти сценарии только загружают и проверяют исходные наборы. Адаптеры наборов и
полный расчёт метрик WIDER FACE, XQLFW и CelebA ещё не реализованы. Существующая
команда `src/run_experiment.py` ожидает уже подготовленный тяжёлый кэш
`faces.csv + embeddings.npy + aligned_faces/`, поэтому запускать её прямо на
каталоге скачанных архивов нельзя.
